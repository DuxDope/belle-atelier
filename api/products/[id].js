...
function checkAuth(req) {
  ...
  try { jwt.verify(auth.replace('Bearer ', ''), process.env.JWT_SECRET); return true; }
  catch { return false; }
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      if (!name || !brand || !cat || !desc) return res.status(400).json({ error: 'Faltan campos' });
      if (!variants || !variants.length) return res.status(400).json({ error: 'Agrega al menos un tono' });

      await client.query('BEGIN');
      await client.query(
        `UPDATE products SET name=$1, brand=$2, cat=$3, description=$4, info=$5, badge=$6, emoji=$7, image_url=$8 WHERE id=$9`,
      ...
      if (toDelete.length) await client.query('DELETE FROM variants WHERE id = ANY($1::int[])', [toDelete]);

      for (const v of variants) {
        if (v.id && existingIds.includes(Number(v.id))) {
          await client.query(
            `UPDATE variants SET tone=$1, price=$2, stock=$3, image_url=$4, image_url2=$5, image_url3=$6 WHERE id=$7`,
            [v.tone, Number(v.price)||0, Number(v.stock)||0, v.image_url||'', v.image_url2||'', v.image_url3||'', Number(v.id)]
          );
        } else {
          await client.query(
            `INSERT INTO variants (product_id, tone, price, stock, image_url, image_url2, image_url3)
             VALUES ($1,$2,$3,$4,$5,$6,$7)`,
            [productId, v.tone, Number(v.price)||0, Number(v.stock)||0, v.image_url||'', v.image_url2||'', v.image_url3||'']
          );
        }
      }
